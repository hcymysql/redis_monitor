	...
	************************************/
	...
	if($is_live==0){
	    ...
	    if($send_weixin==0 || empty($send_weixin)){
            ...
            //-------------------------
	    //$sql = "INSERT INTO redis_status(host,tag,port,role,is_live,create_time)  VALUES('{$ip}','{$tag}','{$port}',{$is_live},now())"; 
		...
		if($send_weixin==0 || empty($send_weixin)){
			...
			$recover_sql = "SELECT is_live FROM mysql_status_history WHERE HOST='{$ip}' AND PORT='{$port}' ORDER BY create_time DESC LIMIT 1";              
			$recover_result =  mysqli_query($con, $recover_sql);
			$recover_row = mysqli_fetch_assoc($recover_result);
		}
		...
		$sql = "INSERT INTO  redis_status(host,tag,port,role,is_live,max_connections,threads_connected,blocked_connected,rejected_connected,qps,maxmemory_human,used_memory_rss_human,used_memory_peak_human,runtime,db_version,aof_enabled,cluster_enabled,Seconds_Behind_Master,create_time) 
		VALUES('{$ip}','{$tag}','{$tcp_port}','{$mode}','{$is_live}','${maxclients}','${connected_clients}','${blocked_clients}','${rejected_connections}','{$qps}','{$maxmemory_human}','{$used_memory_rss_human}','{$used_memory_peak_human}','{$uptime_in_days}','{$redis_version}','{$aof_enabled}','{$cluster_enabled}','${Seconds_Behind_Master}',now())";  
	...
	if (mysqli_query($con, $sql)) {
		echo "\n{$ip}:'{$port}' 新记录插入成功\n";
		echo "-------------------------------------------------------------\n\n\n";
		mysqli_query($con,"INSERT INTO  redis_status_history(host,tag,port,role,is_live,max_connections,threads_connected,blocked_connected,rejected_connected,qps,maxmemory_human,used_memory_rss_human,used_memory_peak_human,runtime,db_version,aof_enabled,cluster_enabled,Seconds_Behind_Master,create_time) 
		VALUES('{$ip}','{$tag}','{$tcp_port}','{$mode}','{$is_live}','${maxclients}','${connected_clients}','${blocked_clients}','${rejected_connections}','{$qps}','{$maxmemory_human}','{$used_memory_rss_human}','{$used_memory_peak_human}','{$uptime_in_days}','{$redis_version}','{$aof_enabled}','{$cluster_enabled}','${Seconds_Behind_Master}',now())");
	
		mysqli_query($con,"DELETE FROM redis_status where host='{$ip}' and port='{$port}' and create_time<DATE_SUB(now(),interval 30 second)");
	} else {
		echo "\n{$ip}:'{$port}' 新记录插入失败\n";
		echo "Error: " . $sql . "\n" . mysqli_error($con);
	}	
